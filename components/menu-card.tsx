...
export function MenuCard({ id, name, description, price, img_url, onDelete, onEdit }: MenuCardProps) {
  return (
    <div className="w-full bg-light-orange rounded-lg p-4 shadow-sm">
      ...
      <div className="mt-3">
        <h3 className="text-lg font-bold text-gray-orange uppercase tracking-wide">{name}</h3>
        <p className="text-xs text-gray-orange mt-1 line-clamp-2">{description}</p>

        <div className="mt-3">
          <div className="w-full text-right text-sm font-bold text-gray-orange">{formatPrice(price)}</div>
        </div>
